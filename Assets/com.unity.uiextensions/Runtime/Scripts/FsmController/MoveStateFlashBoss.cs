...
using UnityEngine;
using DG.Tweening;
public class MoveStateFlashBoss : SlimeStateBase
{
    ...
    public void HandleMove()
    {
         ...
         if(countSkillFlash >= 5)
        {
            if (!data.wasTakeDame)
            {
                ...
                isIdle = true;
                countSkillFlash = 0;
                data.animator.Play("Flash");
                //StartCoroutine(Helper.HandleActionPlayAndWait(data.animator, "Flash", delegate {  Invoke(nameof(HandleMove), 0.5f); }));
                return;
            }
        }    
         else
